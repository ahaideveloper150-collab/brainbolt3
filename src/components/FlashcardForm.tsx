...
interface FlashcardFormProps {
  onSubmit: (data: { content: string; learning_level: string; model: string }) => void;
  isLoading: boolean;
}
...
export default function FlashcardForm({ onSubmit, isLoading }: FlashcardFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmedContent || !selectedLevel) {
      return;
    }
  ...
  const canSubmit = content.trim().length > 0 && learningLevel.length > 0 && (learningLevel !== "Other" || customLevel.trim().length > 0);

  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {/* Content Input */}
      <div>
        <label htmlFor="flashcard-content" className="block text-sm font-medium text-slate-700 mb-2">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-3">
          Which level should the flashcards be designed for?
        </label>
